<template>
  ...
  <GeetestCaptchaWidget
    ...
    @verify="(proof) => emit('verify', JSON.stringify(proof), '')"
    @expire="emit('expire')"
    @error="emit('error')"
  />
</template>
...
import GeetestCaptchaWidget from '@/components/GeetestCaptchaWidget.vue'
...
const geetestRef = ref<InstanceType<typeof GeetestCaptchaWidget> | null>(null)
...
async function verifyAction(): Promise<ActionCaptchaResult | null> {
  ...
  if (props.geetestEnabled && props.geetestCaptchaId) {
    try {
      const proof = (await geetestRef.value?.verify()) ?? null
      if (!proof) return null
      return { token: JSON.stringify(proof), randstr: '' }
    } catch {
      emit('error')
      return null
    }
