      ...
      <table class="w-full min-w-max divide-y divide-gray-200 dark:divide-dark-700">
        ...
        <tbody class="divide-y divide-gray-200 bg-white dark:divide-dark-700 dark:bg-dark-900">
          ...
          <tr
            ...
            @click="$emit('select-user', item.user_id, maskUserIdentity(item.email))"
          >
            ...
            <td class="max-w-[260px] truncate px-4 py-3 text-sm font-medium text-gray-700 dark:text-gray-200" :title="maskUserIdentity(item.email)">
              {{ maskUserIdentity(item.email) || `User #${item.user_id}` }}
