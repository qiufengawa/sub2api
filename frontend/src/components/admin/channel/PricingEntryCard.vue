  ...
  <section class="channel-pricing-entry">
    ...
    <Transition name="ui-collapse"><div v-show="!collapsed" :id="pricingBodyId" class="channel-pricing-entry__body">
      ...
      <template v-if="entry.billing_mode === 'token'">
        ...
        <div v-if="enableTierMultipliers" class="channel-pricing-entry__tier-grid">
          <UiTextField density="mini" :label="t('admin.channels.form.fastMultiplier', 'Fast multiplier')" type="number" min="0.000001" step="any" :model-value="entry.fast_multiplier ?? ''" @update:model-value="emitField('fast_multiplier', String($event))" />
          <UiTextField density="mini" :label="t('admin.channels.form.flexMultiplier', 'Flex multiplier')" type="number" min="0.000001" step="any" :model-value="entry.flex_multiplier ?? ''" @update:model-value="emitField('flex_multiplier', String($event))" />
        </div>
        <div v-if="!hideTokenIntervals" class="channel-pricing-entry__subsection">
          ...
          <div v-if="entry.intervals?.length" class="channel-pricing-entry__intervals"><IntervalRow v-for="(iv, idx) in entry.intervals" :key="idx" :interval="iv" :mode="entry.billing_mode" :enable-multipliers="enableTierMultipliers" @update="updateInterval(idx, $event)" @remove="removeInterval(idx)" /></div>
        </div>
      </template>
...
const { t } = useI18n()
const props = withDefaults(defineProps<{ entry: PricingFormEntry; platform?: string; hideTokenIntervals?: boolean; enableTierMultipliers?: boolean }>(), { hideTokenIntervals: false, enableTierMultipliers: false })
...
function addInterval() { emit('update', { ...props.entry, intervals: [...(props.entry.intervals || []), { min_tokens: 0, max_tokens: null, tier_label: '', input_price: null, output_price: null, cache_write_price: null, cache_read_price: null, input_multiplier: null, output_multiplier: null, cache_write_multiplier: null, cache_read_multiplier: null, per_request_price: null, sort_order: props.entry.intervals?.length || 0 }] }) }
function addMediaTier() { const labels = props.entry.billing_mode === 'video' ? ['480p', '720p', '1080p'] : ['1K', '2K', '4K', 'HD']; const index = props.entry.intervals?.length || 0; emit('update', { ...props.entry, intervals: [...(props.entry.intervals || []), { min_tokens: 0, max_tokens: null, tier_label: labels[index] || '', input_price: null, output_price: null, cache_write_price: null, cache_read_price: null, input_multiplier: null, output_multiplier: null, cache_write_multiplier: null, cache_read_multiplier: null, per_request_price: null, sort_order: index }] }) }
