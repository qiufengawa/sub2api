...
describe('AnnouncementPopup', () => {
  ...
  it('renders mixed Markdown and HTML inside the shared styled container', async () => {
    ...
    const content = document.body.querySelector('.announcement-prose')
  ...
  it('previews an admin announcement without marking it as read', async () => {
    ...
    expect(document.body.querySelector('.announcement-prose h2')?.textContent).toBe('Preview heading')
    expect(document.body.querySelector('.announcement-prose script')).toBeNull()
