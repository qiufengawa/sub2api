...
describe('allSelectedGroupsEnableLongContextPricing', () => {
  it('hides the account override when every selected group enables tier pricing', () => {
    expect(allSelectedGroupsEnableLongContextPricing([1, 2], [group(1, true), group(2, true)])).toBe(true)
  })

  it('keeps the account override when any selected group disables tier pricing', () => {
    expect(allSelectedGroupsEnableLongContextPricing([1, 2], [group(1, true), group(2, false)])).toBe(false)
  })

  it('keeps the account override when selection is empty or incomplete', () => {
