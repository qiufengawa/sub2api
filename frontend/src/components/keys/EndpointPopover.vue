  ...
  <div v-if="allEndpoints.length > 0" class="flex flex-wrap gap-2">
    ...
    >
      ...
      <div class="group/endpoint relative flex items-center gap-1.5">
        <div
          class="pointer-events-none absolute bottom-full left-1/2 z-20 mb-2 w-max max-w-[24rem] -translate-x-1/2 translate-y-1 rounded-xl border border-slate-200 bg-white px-3 py-2.5 text-left opacity-0 shadow-[0_14px_36px_-20px_rgba(15,23,42,0.35)] ring-1 ring-slate-200/80 transition-[opacity,transform] duration-150 group-hover/endpoint:translate-y-0 group-hover/endpoint:opacity-100 group-focus-within/endpoint:translate-y-0 group-focus-within/endpoint:opacity-100 dark:border-slate-700 dark:bg-slate-900 dark:ring-slate-700/70"
        >
          <p
        ...
        <UiIconButton
          type="button"
          density="mini"
          :variant="copiedEndpoint === item.endpoint ? 'success' : 'ghost'"
          :icon="copiedEndpoint === item.endpoint ? 'check' : 'copy'"
          :label="tooltipHint(item.endpoint)"
          @click="copy(item.endpoint)"
        />
