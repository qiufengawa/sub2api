<template><div class="ui-alert" :class="`ui-alert--${tone}`" :role="role || (tone === 'danger' ? 'alert' : 'status')"><Icon :name="icon" size="sm"/><div><strong v-if="title">{{ title }}</strong><p><slot>{{ message }}</slot></p></div><button v-if="dismissible" type="button" aria-label="关闭" @click="emit('dismiss')"><Icon name="x" size="xs"/></button></div></template>
