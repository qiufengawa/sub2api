<template>
  <UiFormField :label="label" :description="description" :error="error">
    <div class="ui-entity-picker" @focusin="open=true" @focusout="closeLater">
      <UiSearchInput :model-value="query" :placeholder="placeholder" :disabled="disabled" @update:model-value="search" />
      <div v-if="open && query && (loading || items.length || emptyText)" class="ui-entity-picker__results" role="listbox">
        <p v-if="loading" role="status"><UiSpinner size="sm" />正在搜索</p>
...
const props=withDefaults(defineProps<{modelValue?:string|number|null;items:UiEntityOption[];selectedLabel?:string;label?:string;description?:string;error?:string;placeholder?:string;emptyText?:string;loading?:boolean;disabled?:boolean}>(),{placeholder:'搜索用户、账号或密钥',emptyText:'没有匹配结果'})
...
function select(item:UiEntityOption){query.value=item.label;open.value=false;emit('update:modelValue',item.value);emit('select',item)}
