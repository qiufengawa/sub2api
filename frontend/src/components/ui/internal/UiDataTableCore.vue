<template>
  <div v-if="!isDesktopViewport && !mobileTable" class="space-y-2" :aria-busy="loading">
    ...
    <template v-else-if="!data || data.length === 0">
      <div class="rounded-[4px] border border-gray-200 bg-white p-6 text-center dark:border-dark-700 dark:bg-dark-900">
        <slot name="empty">
          <div class="flex flex-col items-center">
            <Icon
              name="inbox"
              size="xl"
              class="mb-3 h-8 w-8 text-gray-400 dark:text-dark-500"
            />
    ...
    <template v-else>
      ...
      <div
        ...
        class="rounded-[4px] border border-gray-200 bg-white p-3 dark:border-dark-700 dark:bg-dark-900"
        :class="{
    ...
    <table class="w-full min-w-max divide-y divide-gray-200 dark:divide-dark-700">
      <thead class="table-header bg-gray-50 dark:bg-dark-800">
        <tr>
          <th
            v-if="selectable"
            scope="col"
            class="sticky-header-cell w-10 min-w-10 px-2 py-2 text-center"
          >
            <input
              type="checkbox"
              class="h-4 w-4 rounded border-gray-300 text-primary-600 focus:ring-primary-500 dark:border-dark-600 dark:bg-dark-800"
          ...
          >
            ...
            >
              ...
              <span
                ...
                aria-hidden="true"
              >
                <Icon name="chevronUp" size="xs" :class="getSortIndicatorClass(column.key, 'asc')" />
                <Icon name="chevronDown" size="xs" :class="getSortIndicatorClass(column.key, 'desc')" />
              </span>
            </component>
          </th>
        </tr>
      ...
      <tbody class="table-body divide-y divide-gray-200 bg-white dark:divide-dark-700 dark:bg-dark-900">
        ...
        <tr v-if="loading" v-for="i in 5" :key="i">
          <td v-if="selectable" class="w-10 min-w-10 px-2 py-2">
        ...
        <tr v-else-if="!data || data.length === 0">
          <td
            ...
            :class="['py-8 text-center text-gray-500 dark:text-dark-400', getAdaptivePaddingClass()]"
          >
            <slot name="empty">
              <div class="flex flex-col items-center">
                <Icon
        ...
        <template v-else>
          ...
          >
            <td v-if="selectable" class="w-10 min-w-10 px-2 py-2 text-center">
              <input
                type="checkbox"
                class="h-4 w-4 rounded border-gray-300 text-primary-600 focus:ring-primary-500 dark:border-dark-600 dark:bg-dark-800"
            ...
            <td
              ...
              :key="column.key"
              :class="[
                'whitespace-nowrap py-2 text-sm text-gray-900 dark:text-gray-100',
                getAdaptivePaddingClass(),
...
import type { Column } from '@/components/ui/dataTableTypes'
...
onMounted(() => {
  if (typeof window !== 'undefined') {
    ...
    } else if (typeof desktopViewportMediaQuery.addListener === 'function') {
...
onUnmounted(() => {
  ...
  if (desktopViewportMediaQuery && desktopViewportListener) {
    ...
    } else if (typeof desktopViewportMediaQuery.removeListener === 'function') {
...
interface Props {
  ...
  /** Estimated row height in px for the compact virtualized table (default 40) */
...
const getAdaptivePaddingClass = () => {
  ...
  if (columnCount >= 10) {
    ...
    return 'px-3' // 紧凑表格统一保留 12px 水平内边距
  }
}
