<template>
  <div class="ui-pagination">
    <span class="ui-pagination__summary">{{ from }}-{{ to }} / {{ total }}</span>
    ...
    <nav aria-label="分页">
      <button type="button" :disabled="page <= 1" aria-label="上一页" @click="go(page - 1)"><Icon name="chevronLeft" size="sm" /></button>
      <button
      ...
      >{{ item }}</button>
      <button type="button" :disabled="page >= totalPages" aria-label="下一页" @click="go(page + 1)"><Icon name="chevronRight" size="sm" /></button>
    </nav>
    <form v-if="showJump" class="ui-pagination__jump" @submit.prevent="submitJump">
      <span>跳至</span><input v-model="jump" type="number" min="1" :max="totalPages" aria-label="页码"><button type="submit">确定</button>
    </form>
  </div>
</template>
...
const props = withDefaults(defineProps<{
  ...
  showJump?: boolean
}>(), { pageSizeOptions: () => [10, 20, 50, 100], showPageSizeSelector: true, showJump: false })
...
function changePageSize(event: Event) { emit('update:pageSize', Number((event.target as HTMLSelectElement).value)); emit('update:page', 1) }
