<template>
  <button :type="type" :disabled="disabled" class="ui-icon-button ui-focus-ring ui-motion" :class="[`ui-icon-button--${density}`,`ui-icon-button--${variant}`]" :aria-label="label" :title="tooltip || label">
    <slot><Icon v-if="icon" :name="icon" size="sm" /></slot>
  </button>
</template>
...
height:32px}.ui-icon-button--default{width:32px;
height:32px}.ui-icon-button--mini{width:24px;height:24px}.ui-icon-button--large{width:40px;height:40px}
</style>
