    ...
    admin: {
      ...
      paymentConfigDesc: '管理支付服务商与相关设置',
	      ...
	      catalogImport: {
	        ...
	        templateSuccess: '模板已写入 {count} 个可用账号来源分组，并补充 {routes} 条必要的模型路由',
	        templatePartial: '模板已写入 {count} 个账号来源和 {routes} 条模型路由；{failed} 个来源读取失败，另有 {omitted} 项因安全上限或不兼容通配格式未写入，请在导入预览中重点检查',
	        templateNoSources: '未找到含账号的启用分组；模板已下载，导入后需手动绑定账号',
	        ...
	        securityNotice: '仅处理套餐目录配置，不导入支付密钥、上游令牌、用户、API 密钥、订单、用量或历史订阅。下载模板时会自动填入当前可用的账号来源分组。',
	        fileMode: '上传文件',
	        pasteMode: '粘贴 JSON',
