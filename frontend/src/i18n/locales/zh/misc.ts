export default {
  ...
  customPage: {
    ...
    settingsLoadFailedDesc: '自定义页面配置加载失败，请重试。',
  ...
  userSubscriptions: {
    ...
    failedToLoad: '加载订阅失败',
    failedToLoadDescription: '请检查网络连接后重试。',
    status: {
      active: '使用中',
      expired: '已过期',
      revoked: '已撤销',
  ...
  onboarding: {
    ...
    admin: {
      ...
      accountPriority: {
        title: ' 4. 优先级（可选）',
        description:
          '<div style="line-height: 1.7;"><p style="margin-bottom: 12px;">设置账号的调用优先度。</p><div style="padding: 8px 12px; background: #eff6ff; border-left: 3px solid #3b82f6; border-radius: 4px; font-size: 13px; margin-bottom: 12px;"><b>优先度规则：</b><ul style="margin: 8px 0 0 16px;"><li>数字越大，调用优先度越高</li><li>0 为最低优先度</li><li>相同优先度继续按负载和平台调度规则选择</li></ul></div><p style="padding: 8px 12px; background: #f0fdf4; border-left: 3px solid #10b981; border-radius: 4px; font-size: 13px;"><b>使用场景：</b>主账号设置较高数值，备用账号设置较低数值</p></div>',
        nextBtn: '下一步'
      },
      accountGroups: {
        title: ' 5. 分配分组',
  ...
  payment: {
    ...
    result: {
      ...
      refundedHint: '该订单已完成全额退款。',
      partiallyRefundedHint: '该订单已完成部分退款。',
      refundFailedHint: '最近一次退款尝试失败，请核对订单后重试。',
      receiptTitle: '订单回执',
      expectedCredit: '预计到账',
      paidAt: '支付时间',
      completedAt: '完成时间',
    ...
    renewalTarget: '正在续费的订阅',
    subscriptionInstance: '订阅实例 #{id}',
    ...
    errors: {
      ...
      renewalTargetUnavailable: '所选续费订阅实例已不可用，请重新选择套餐。',
    ...
    admin: {
      ...
      validity: '有效期',
      validityUnit: '有效期单位',
	  maxSubscriptionsPerUser: '单用户最大订阅份数',
	  maxSubscriptionsPerUserHint: '1 表示同档套餐只能持有一份；大于 1 时可重复订阅，每份期限和额度独立。',
      maxSubscriptionsPerUserInvalid: '最大订阅份数必须是 1 到 2147483647 的整数',
      sortOrder: '排序',
      forSale: '上架状态',
      onSale: '上架',
      offSale: '下架',
      ...
      paymentConfigDesc: '管理支付服务商与相关设置',
	      ...
	      catalogImport: {
	        ...
	        fields: {
	          ...
	          max_subscriptions_per_user: '单用户最大订阅份数',
	          value: '值',
	        },
	        issueCodes: {
