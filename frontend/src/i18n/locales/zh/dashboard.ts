export default {
  dashboard: {
    ...
    overview: {
      title: '用户概览',
      status: { healthy: '服务正常', warning: '需要关注', danger: '服务受限' },
      headerNoKey: '尚未创建 API 密钥，完成接入后即可开始调用。',
      headerReady: 'API 密钥已就绪，现在可以发送第一个请求。',
      headerHealthy: '服务运行正常，最近一次成功调用于 {time}。',
      headerDefault: '查看账户额度、接入状态与消费情况。',
      headerDataSummary: '汇总今日与累计用量，查看近期趋势、模型占比和请求记录。',
      headerAccountDisabled: '当前账户已停用，请联系管理员或检查账户状态。',
      headerLowBalance: '余额不足，服务可能受到影响。',
      headerQuotaNearLimit: '订阅或平台额度即将用尽，请及时处理。',
      rechargeOrRedeem: '充值或兑换',
      topUpOrSubscribe: '充值或订阅',
      redeemBalance: '兑换额度',
      availableBalance: '可用余额',
      subscriptionAllowance: '可用订阅额度',
      subscriptionBilling: '订阅计费',
      todayDeduction: '今日扣费 {amount}',
      ...
      allTime: '全部历史',
      totalUsageCost: '累计消费',
      totalUsageRequests: '累计请求',
      standardCost: '标准费用 {amount}',
      balanceRemaining: '可用余额 {amount}',
      liveRequests: '当前 {rpm} RPM · {tpm} TPM',
      inputOutputTokens: '输入 {input} · 输出 {output}',
      averageResponseTime: '平均响应 {duration}',
      requestCount: '{count} 次请求',
      tokenCount: '{count} Token',
      moreThanYesterday: '较昨日增加 {percent}%',
      lessThanYesterday: '较昨日减少 {percent}%',
      accessStatus: '接入状态',
      noApiKeyTitle: '尚未创建 API Key',
      noApiKeyDescription: '创建密钥后即可开始调用。',
      keysAvailable: '{active} / {total} 个密钥可用',
      lastSuccess: '最近成功 {time}',
      noSuccessfulCalls: '还没有成功调用',
      recentFailures: '24 小时失败 {count} 次',
      usageTrend: '费用与请求趋势',
      periodSummary: '实际扣费 {cost} · {requests} 次请求',
      periodComparison: '较上一周期：费用 {cost} · 请求 {requests}',
      newInPeriod: '新增',
      last7Days: '近 7 天',
      last30Days: '近 30 天',
      actualCost: '实际扣费',
      requests: '请求数',
      noTrendTitle: '还没有趋势数据',
      noTrendDescription: '完成首次 API 调用后，这里会显示费用和请求变化。',
      viewApiKeys: '查看 API 密钥',
      allowanceStatus: '额度与订阅',
      allowanceStatusDescription: '优先显示最接近上限的额度',
      accountBalance: '账户余额',
      manage: '管理',
      noAllowanceTitle: '当前没有限额项目',
      noAllowanceDescription: '账户按余额正常计费，订阅或平台额度配置后会显示在这里。',
      daily: '日额度',
      weekly: '周额度',
      monthly: '月额度',
      disabled: '已禁用',
      expiresAt: '{date} 到期',
      resetsAt: '{date} 重置',
      costBreakdown: '模型费用构成',
      costBreakdownDescription: '按实际扣费从高到低排列',
      modelShare: '模型用量占比',
      modelShareDescription: '按 Token 用量统计近期模型构成',
      modelRowMeta: '{requests} 次 · {cost}',
      details: '明细',
      otherModels: '其他模型',
      noCostTitle: '还没有费用数据',
      noCostDescription: '产生调用后即可查看主要消费模型。',
      recentActivity: '近期活动与异常',
      recentActivityDescription: '最近的成功调用和失败请求',
      noActivityTitle: '还没有调用记录',
      noActivityDescription: '发送第一个请求后，活动会显示在这里。',
      successActivityMeta: '{cost} · 响应 {duration}',
      requestFailed: '请求失败',
      viewFailedRequests: '查看失败请求',
      gettingStartedTitle: '开始接入',
      gettingStartedDescription: '完成下面三步即可开始使用 API，不再用一整页空统计打断你。',
      firstRequestTitle: '发送第一个请求',
      firstRequestDescription: '密钥已经准备好。复制接口地址和请求示例，验证接入是否正常。',
      stepCreateKey: '创建 API 密钥',
      stepChooseChannel: '查看可用模型与渠道',
      stepSendRequest: '发送第一个请求',
      apiEndpoint: 'API 接口地址',
      requestExample: '请求示例',
      copy: '复制',
      copied: '已复制',
      copyRequest: '复制请求',
      fullDocumentation: '查看完整文档',
      viewAvailableModels: '查看可用模型',
      completed: '已完成',
      alertAccountDisabled: '当前账户已被停用，API 服务不可用。',
      alertNoActiveKey: '没有可用的 API 密钥，请启用或创建密钥。',
      alertLowBalance: '当前余额不足且没有有效订阅，调用可能无法继续。',
      alertQuota: '有一项额度已使用 {percent}%，请留意剩余额度。',
      alertFailures: '最近 24 小时有 {count} 次失败请求，建议检查错误详情。',
      alertExpiry: '订阅将在 {days} 天内到期，请及时续订。',
      viewProfile: '查看账户',
      manageKeys: '管理密钥',
      viewSubscriptions: '查看订阅',
      viewUsage: '查看用量',
      loadFailedTitle: '用户概览加载失败',
      loadFailedDescription: '暂时无法读取账户数据，请稍后重试。',
      retry: '重试'
    }
  },
  ...
  keys: {
    ...
    subscriptionIntent: {
      createKey: '创建订阅密钥',
      createTitle: '创建订阅密钥',
      createDescription: '已为你预选套餐覆盖的路由分组“{group}”。请确认密钥名称和其他设置后创建。',
      bindTitle: '绑定现有密钥',
      bindDescription: '请选择一把密钥绑定到套餐覆盖的路由分组“{group}”。你可以继续使用搜索、筛选和分页查找。',
      bindAction: '绑定',
      alreadyBound: '已绑定',
      confirmTitle: '确认更换密钥分组',
      confirmMessage: '密钥“{key}”将从“{current}”更换到“{target}”。这会改变该密钥可使用的模型、线路和计费来源，正在使用此密钥的请求可能受到影响。',
      confirmAction: '确认绑定',
      bindSuccess: '密钥“{key}”已绑定到“{group}”',
      bindFailed: '绑定密钥失败，请确认订阅仍然有效后重试',
      noKeys: '暂无可绑定的密钥',
      noKeysDescription: '你还没有 API 密钥，可以直接创建一把已预选该套餐覆盖路由分组的新密钥。',
      invalid: '订阅密钥操作参数无效，请从“我的订阅”重新进入',
      unavailable: '该套餐覆盖的路由分组当前不可用，请刷新“我的订阅”后重试',
      groupsLoadFailed: '无法读取可用分组，暂时不能继续订阅密钥操作',
    },
    editKey: '编辑密钥',
    deleteKey: '删除密钥',
    ...
    useKeyModal: {
      ...
      routedCodex: {
        description: '使用当前分组的 API Key 和模型目录配置 Codex。',
        configTomlHint: '先获取并下载模型目录，再将目录和 config.toml 保存到 Codex 配置目录。',
  ...
  usage: {
    ...
    cacheHitRateShort: '缓存',
    cacheHitRateFormula: '缓存读取 ÷（普通输入 + 缓存创建 + 缓存读取）',
    totalCost: '总消费',
    standardCost: '标准',
    actualCost: '实际',
    accountCost: '成本',
    ...
    queryConditionsHint: '调整条件后点击查询，统计、图表和日志会统一刷新。',
    queryAction: '查询',
    appliedConditions: '当前范围',
    rankingOther: '其他',
    moreFilters: '更多筛选',
    exportCsv: '导出 CSV',
    ...
    billingFallbackReason: '兜底原因',
    billingSourceSubscription: '订阅积分',
    billingSourceWallet: '账户余额',
    billingSourceLegacy: '历史规则',
    billingPreferenceSubscriptionFirst: '订阅优先',
    billingPreferenceWalletFirst: '余额优先',
    billingPreferenceSubscriptionOnly: '仅订阅',
    billingPreferenceWalletOnly: '仅余额',
    billingFallbackWalletInsufficient: '余额不足，改用订阅',
    billingFallbackBothUnavailable: '订阅不可用，余额继续扣费',
    billingFallbackSubscriptionExhausted: '订阅额度不足，改用余额',
    billingFallbackSubscriptionUnavailable: '无可用订阅，改用余额',
    firstToken: '首 Token',
    duration: '耗时',
    latency: '延迟',
  ...
  availableChannels: {
    ...
    resultCount: '共 {count} 个渠道',
    expandDetails: '展开渠道详情',
    collapseDetails: '收起渠道详情',
    empty: '暂无可用渠道',
    noModels: '未配置模型',
    noPricing: '未配置定价',
    exclusive: '专属',
  ...
  affiliate: {
    ...
    transfer: {
      ...
      empty: '当前没有可转入额度',
      confirmTitle: '确认转入余额',
      confirmMessage: '确定将全部可用返利额度 {amount} 转入账户余额吗？转入后无法撤销。',
      confirmButton: '确认转入',
  ...
  profile: {
    ...
    linkedProfileSourcesDescription: '部分头像和昵称可能同步自第三方登录方式。',
	    securityTitle: '安全设置',
	    securityDescription: '集中管理密码、双因素认证和 Passkey。',
	    billingPreference: {
	      title: '扣费顺序',
	      description: '选择同时拥有订阅积分和账户余额时的扣费方式。单次请求不会拆分到两个资金来源。',
	      subscriptionFirst: '订阅积分优先',
	      subscriptionFirstDesc: '优先使用覆盖当前分组的订阅积分；积分不足时按套餐设置决定是否扣余额。',
	      walletFirst: '账户余额优先',
	      walletFirstDesc: '余额足够时先扣余额，余额不足时再尝试使用可用订阅积分。',
	      subscriptionOnly: '仅订阅积分',
	      subscriptionOnlyDesc: '只允许从订阅积分扣费；额度不足时直接拒绝请求，不扣账户余额。',
	      walletOnly: '仅账户余额',
	      walletOnlyDesc: '忽略订阅积分，所有请求均从账户余额扣费。',
	    },
    administrator: '管理员',
    user: '用户',
    username: '用户名',
