export default {
  accounts: {
    ...
    dataExportedSkippedShadows:
      "数据已导出。已跳过 {count} 个 spark 影子账号：其调度配置不在备份内，还原后需在重建的影子上重新调优。",
    ...
    dataImportPriorityCompatibility:
      "合法的旧版 v1 备份会按其旧优先级语义与转换基准自动迁移；新版 v2 备份会原样保留。",
    dataImportLegacyPriorityMigrated:
      "检测到旧版优先级语义，已在导入时转换为“数字越大越优先”。",
    dataImportLegacyPriorityPreview:
      "已选择旧版 v1 备份。导入时会使用文件内声明的转换基准迁移为“数字越大越优先”。",
    dataImportInvalidPriorityFile:
      "文件 {name} 中的调用优先度必须使用规范十进制整数，且不能为负数、小数、指数、字符串或超出数据库范围。",
    dataImportMixedVersions: "不能同时合并不同版本的备份文件，请分开导入。",
    dataImportMixedPrioritySemantics:
      "不能同时合并调用优先度语义不同的备份文件，请分开导入。",
    dataImportMixedPriorityPivots:
      "旧版备份的优先级转换基准不同，不能合并导入，请分别导入。",
    dataImportFile: "数据文件",
    ...
    serviceStatus: {
      title: "近 60 分钟服务状态",
      passiveHint:
        "仅根据真实用户调用记录统计，不会主动检测账号，也不会产生额外请求或费用。",
    ...
    schedulerScore: {
      ...
      stickyShort: "粘性",
      ungrouped: "未分组",
      hint: "显示格式为“分组名 / 层内基础分 / 粘性加分”。只有最高调用优先度层参与普通调度，低层分数不能跨层反超。该视图是静态快照，未采集的实时错误率和首包延迟不会伪装成实测值；粘性加分只在开启粘性加权时生效。",
    },
    usageWindowsHint:
    ...
    ollamaCloud: {
      ...
      sessionSecurityHint:
        "浏览器会话会加密落库，且只发送到固定的 Ollama 官方设置页。",
      configured: "已配置",
      notConfigured: "未配置",
      notRefreshed: "尚未刷新",
      encryptionKeyRequired:
        "请先配置持久 TOTP_ENCRYPTION_KEY，再保存浏览器会话。",
      ...
      errors: {
        ...
        OLLAMA_CLOUD_USAGE_REFRESH_RATE_LIMITED:
          "刷新过于频繁，请在 {retry_after_seconds} 秒后重试。",
      },
    },
    upstreamBilling: {
      trustWarning:
        "此倍率由上游站点针对当前 API Key 自行声明。Sub2API 无法验证该值是否与实际扣费一致；上游站点或中间代理可能返回伪造、过期或被篡改的数据。请结合账单、余额变化和实际用量自行核验。",
      autoProbe: "自动探测上游声明倍率",
      autoProbeHint:
        "启用后按全局周期刷新上游声明倍率；此开关本身不会修改账号倍率。",
      syncRate: "同步上游声明倍率",
      syncRateHint:
        "成功探测后自动更新账号倍率，同步的是不含高峰的基准倍率；探测失败或声明超出允许范围时保持不变。开启本项会同时打开“自动探测上游声明倍率”。",
    ...
    tempUnschedulable: {
      ...
      multipleErrorTriggerNoWindow:
        "累计 {count} 次匹配错误，达到触发阈值（{threshold}）。",
      multipleErrorCountInWindow:
        "{minutes} 分钟内累计发生 {count} 次匹配错误。",
    ...
    usageWindow: {
      ...
      grokFreeQuota24hHint:
        "按 sub2api 近 24 小时本地 Token 用量估算（上限 {limit}）",
    ...
    openaiQuotaReset: {
      ...
      resetCacheRefreshFailed:
        "窗口已重置、账号状态已恢复，但重置次数未能回读，请重新查询次数。",
      resetAccountRecoveryFailed:
        "窗口已重置，但账号状态恢复失败，请手动恢复账号状态。",
      resetAccountRefreshFailed:
        "窗口、账号状态和重置次数缓存已更新，但无法加载最新账号显示。",
      refreshCachePersistFailed:
        "已显示实时次数，但到期明细获取失败，仍保留原有缓存明细。",
      confirmTitle: "确认重置周限",
      confirmMessage:
        "将消耗 1 次重置次数立即恢复当前窗口，剩余 {count} 次。此操作不可撤销，确定继续吗？",
    },
    tier: {
      free: "Free",
      pro: "Pro",
    ...
    bulkEdit: {
      ...
      baseUrlNotice:
        "适用于 API Key 账号及 Grok OAuth 账号的转发端点，留空则不修改",
      submit: "批量更新",
      updating: "更新中...",
      ...
      rateSyncWarning:
        "已开启上游倍率同步的账号不能批量手工修改倍率，请先在账号编辑页关闭同步。",
      rateSyncConflict:
        "无法修改账号倍率：{count} 个目标账号已开启上游倍率同步。",
    ...
    createSparkShadowConfirm:
      "为「{name}」创建链接型 Spark 影子账号?影子共享母账号凭据、仅服务 spark 模型。",
    ...
    openai: {
      ...
      responsesModeTextDisabledHint:
        "未启用 Responses / Chat Completions 端点时，此设置不适用。",
      ...
      planType: "订阅档位（手动覆盖）",
      planTypeDesc:
        "手动纠正本账号的 ChatGPT 订阅档位（Plus / Pro / Free）。注意：令牌临期刷新或命中 429 限流时，会用真实档位自动覆盖此处设置。",
      ...
      codexFingerprintModeDesc:
        "多人共享同一 OAuth 账号时，将各用户的设备和会话标识收敛为账号级稳定值，减少上游可见的设备数和会话数；关闭时原样透传客户端标识。",
      codexFingerprintOff: "关闭（透传）",
      codexFingerprintDevice: "仅设备",
      codexFingerprintSession: "设备和会话（推荐）",
      ...
      codexImageToolInheritDesc:
        "不写入账号覆盖；非 Lite 请求是否注入 hosted 工具由渠道或全局策略决定，客户端显式携带的 hosted 工具和本地 image_gen 声明照常放行。",
    ...
    grok: {
      ...
      apiKeyHint:
        "Grok 订阅支持使用 OAuth refresh token；API Key 账号不在本次范围内。",
      testMode: "测试模式",
      testModeHint:
        "文本、图片和视频使用所选模型；网页搜索、TTS、STT、Realtime 使用独立接口探测。",
      ...
      searchTestHint: "独立网页搜索探测，不是带工具的自由对话。",
      ttsTextLabel: "TTS 文本",
      ttsTextPlaceholder: "例如：Hello from Sub2API connectivity test.",
      ttsTextDefault: "Hello from Sub2API account connectivity test.",
      ttsTestHint: "独立调用 /v1/tts；成功时显示音频结果。",
      sttTestHint: "独立调用 /v1/stt；成功表示接口可达。",
      realtimeTestHint: "独立连接 /v1/realtime WebSocket；握手成功即表示连通。",
      sendingSearchRequest: "正在发送独立 web_search 请求...",
      sendingTTSRequest: "正在发送独立 /tts 请求...",
      sendingSTTRequest: "正在发送独立 /stt 请求...",
      sendingRealtimeRequest: "正在连接独立 /realtime WebSocket...",
      selectedTestMode: "测试模式：{mode}",
      imageUploadLabel: "源图片（可选，图生图或编辑）",
      videoFirstFrameLabel: "首帧或参考图（可选）",
      imageUploadHint:
        "建议使用 PNG 或 JPEG。上传源图时进行图生图或编辑；不上传时进行文生图。",
      videoFirstFrameHint: "可选首帧或参考图，用于图生视频。",
      audioUploadLabel: "音频文件（STT 可选）",
      audioUploadHint: "上传真实音频进行转写；不上传时仅测试接口连通性。",
      mediaTooLarge: "文件过大，管理端测试上传上限约为 6 MB。",
    ...
    autoPauseThresholdHint:
      "留空或填 0 表示使用全局默认阈值（在运维设置中配置）；填具体值则覆盖全局默认。达到阈值后仅在调度时跳过账号，不修改 schedulable。",
    autoPause5hDisabled: "禁用 5h 自动暂停",
    autoPause7dDisabled: "禁用 7d 自动暂停",
    autoPauseDisabledHint:
      "开启后该账号永不进入自动暂停（即使全局默认阈值已配置）。",
    ...
    priority: "调用优先度",
    priorityHint: "数字越大，调用优先度越高；相同数值继续按负载等平台规则调度，0 为最低优先度",
    priorityInvalid: "调用优先度必须是 0 或更大的十进制整数，且不能超过数据库整数范围",
    priorityColumnHint:
      "数字越大越优先进入调度；同值仍按负载、队列、最近使用时间、配额和平台策略选择。该数值不是流量百分比，也不保证每次选中同一账号",
    priorityDecrease: "降低调用优先度",
    priorityIncrease: "提高调用优先度",
    prioritySaving: "正在保存调用优先度",
    priorityUpdateFailed: "调用优先度保存失败",
    accountSchedulingThresholdOverride: "账号自动停调阈值覆盖",
    accountSchedulingThresholdOverrideHint:
      "仅覆盖当前账号的平台自动停调阈值；关闭后使用平台设置。",
    accountSchedulingThresholdOverrideValue: "账号阈值百分比",
    accountSchedulingThresholdOverrideDisabledHint:
      "填写 1 到 100。达到该用量百分比后账号将临时停止调度；100 表示对当前账号禁用自动停调。",
    billingRateMultiplier: "账号计费倍率",
    billingRateMultiplierHint: "0 表示不计费，仅影响账号计费",
    expiresAt: "过期时间",
    expiresAtHint: "留空表示不过期",
    higherPriorityFirst: "数值越大调用优先度越高",
    ...
    oauth: {
      ...
      openai: {
        ...
        agentIdentityInvalid:
          "请选择 auth_mode=agentIdentity 的 Codex auth.json。",
        codexPatAuth: "Codex Personal Access Token",
        codexPatDesc:
          "输入 Codex at- Personal Access Token，系统会先调用 OpenAI whoami 校验后再创建账号。",
        codexPatInputLabel: "Codex PAT",
        codexPatPlaceholder: "at-...",
        codexPatHint:
          "这是独立认证模式，不保存 refresh_token，也不会写入 OAuth access_token 过期时间。",
      ...
      grok: {
        ...
        importantNotice:
          "当浏览器跳转到本地 callback URL 后，请复制完整 URL 或 code 参数回填到这里。",
        step3EnterCode: "输入授权链接或 Code",
        authCodeDesc: "授权完成后，粘贴 callback URL、查询字符串或授权码：",
        authCode: "授权链接或 Code",
        authCodePlaceholder:
          "粘贴完整 callback URL、?code=... 查询字符串或 code 值",
        authCodeHint: "支持完整 callback URL、查询字符串或裸 code。",
        refreshTokenAuth: "手动输入 RT",
        refreshTokenDesc:
          "输入已有的 xAI refresh token，支持批量输入（每行一个）。",
        refreshTokenPlaceholder:
          "粘贴您的 xAI refresh token...\n支持多个，每行一个",
        ssoCookieAuth: "SSO Cookie 导入",
        ssoCookieDesc:
          "每行粘贴一个 Grok Web SSO key，系统会自动走 xAI Device Flow 并转换为 Grok Build OAuth 凭据。",
        ssoCookieLabel: "Grok Web SSO Key",
        ssoCookiePlaceholder: "每行一个 SSO key\n支持多个，每行一个",
        ssoCookieHint:
          "每行一个 SSO key；多个 key 会 3 路并发导入，耗时约 90 秒 × 批次数，建议使用对应地区代理。",
        emailPasswordAuth: "邮箱密码登录",
        emailPasswordDesc:
          "使用 Grok 网页邮箱和密码登录。服务端仅使用密码获取临时 SSO 并转换为 OAuth 凭据，不会保存密码或原始 SSO。",
        emailPasswordInputLabel: "邮箱----密码",
        emailPasswordPlaceholder:
          "邮箱----密码\n支持多行，每行一组",
        emailPasswordHint:
          "格式：email----password（密码可以包含连字符）。需要配置 YesCaptcha 密钥，建议使用匹配地区的代理。",
        ...
        oauthOnlyHint:
          "首版 Grok 支持仅包含 OAuth 订阅的 Responses API 文本/推理转发。",
      },
      // Gemini specific
      gemini: {
        title: "Gemini 账户授权",
    ...
    videoPromptPlaceholder: "例如：一只红球在白色地板上弹跳一次。",
    videoPromptDefault:
      "A red ball bouncing once on a white floor, short simple motion.",
    videoTestHint: "调用视频生成接口，完成后在页面中预览结果。",
