export default {
  accounts: {
    ...
    dataExportedSkippedShadows:
      "Data exported. Skipped {count} spark shadow account(s): their scheduling config is not included in the backup; recreate and re-tune them after restore.",
    ...
    dataImportPriorityCompatibility:
      "Valid v1 backups are migrated using their declared legacy priority semantics and conversion pivot; v2 backups keep their values.",
    dataImportLegacyPriorityMigrated:
      "Legacy priority semantics were detected and converted to “higher values are used first”.",
    dataImportLegacyPriorityPreview:
      "A legacy v1 backup is selected. Import will migrate it to higher-values-first using the conversion pivot declared in the file.",
    dataImportInvalidPriorityFile:
      "Call priorities in {name} must be canonical decimal integers, not negative values, decimals, exponents, strings, or values outside the database range.",
    dataImportMixedVersions:
      "Backups with different versions cannot be merged. Import them separately.",
    dataImportMixedPrioritySemantics:
      "Backups with different call-priority semantics cannot be merged. Import them separately.",
    dataImportMixedPriorityPivots:
      "Legacy backups with different priority conversion pivots cannot be merged. Import them separately.",
    ...
    crsVersionRequirement:
      " Note: CRS version must be ≥ v1.1.240 to support this feature",
    ...
    serviceStatus: {
      ...
      passiveHint:
        "Calculated only from real user requests. It never probes accounts or creates extra requests or charges.",
      historyLabel:
        "Service health for the last 60 minutes, one minute per bar, oldest on the left and newest on the right",
      hourAgo: "60 min ago",
      now: "Now",
    ...
    schedulerScore: {
      ...
      stickyShort: "Sticky",
      ungrouped: "Ungrouped",
      hint: 'Displayed as "group / within-tier base score / sticky bonus". Only the highest call-priority tier participates in ordinary scheduling, so lower-tier scores cannot overtake it. This is a static snapshot; unavailable live error-rate or first-token metrics are not presented as measured values. Sticky bonus applies only when sticky weighting is enabled.',
    },
    usageWindowsHint:
    ...
    ollamaCloud: {
      ...
      sessionSecurityHint:
        "The browser session is encrypted at rest and sent only to the fixed official settings URL.",
      configured: "Configured",
      notConfigured: "Not configured",
      notRefreshed: "Not refreshed",
      encryptionKeyRequired:
        "Set a persistent TOTP_ENCRYPTION_KEY before storing a browser session.",
      sessionLabel: "Ollama browser Cookie",
      sessionPlaceholder:
        "wos-session=...; __Secure-authjs.session-token.0=...",
      writeOnlyHint:
        "Write-only. The saved value cannot be viewed and an empty value never replaces it.",
      deleteSession: "Delete session",
      deleteConfirm:
        "Delete the stored Ollama browser session and its usage snapshot?",
      refreshNow: "Refresh usage",
      autoRefresh: "Automatic usage refresh",
      autoRefreshHint:
        "Runs only when the account switch and the global switch are both enabled.",
      plan: "Plan",
      fiveHour: "5 hour",
      fiveHourShort: "5h",
      sevenDay: "7 day",
      ...
      errors: {
        ...
        OLLAMA_CLOUD_USAGE_REFRESH_RATE_LIMITED:
          "Refresh is limited. Try again in {retry_after_seconds} seconds.",
      },
    },
    upstreamBilling: {
      trustWarning:
        "This rate is declared by the upstream site for the current API key. Sub2API cannot verify that it matches actual charges. The upstream site or an intermediary may return forged, stale, or modified data. Verify it against bills, balance changes, and actual usage.",
      autoProbe: "Automatically probe upstream declared rate",
      autoProbeHint:
        "Refresh the upstream declared rate on the global interval. This switch alone does not change the account rate.",
      syncRate: "Sync upstream declared rate",
      syncRateHint:
        'Update the account rate after each successful probe, using the base rate excluding peak hours. Failed probes or declarations outside the allowed range leave it unchanged. Enabling this also turns on "Automatically probe upstream declared rate".',
      syncRateManagedHint:
        "The current rate is maintained automatically from the upstream declared base rate (excluding peak hours).",
      syncedRateTooltip:
        "This account rate is synchronized from the upstream declared base rate (excluding peak hours)",
      ...
      batchPartial:
        "Probe partially completed: {success} succeeded, {failed} failed",
    },
    ...
    tempUnschedulable: {
      ...
      multipleErrorTriggerNoWindow:
        "{count} matching errors reached the trigger threshold ({threshold}).",
      multipleErrorCountInWindow:
        "{count} matching errors occurred within {minutes} minutes.",
    ...
    bulkActions: {
      ...
      selectAllFailed:
        "Failed to load all accounts. The previous selection was kept.",
    ...
    bulkEdit: {
      ...
      baseUrlNotice:
        "Applies to API Key accounts and the forwarding endpoint of Grok OAuth accounts; leave empty to keep existing value",
      ...
      rateSyncWarning:
        "Accounts with upstream rate sync enabled cannot be changed in bulk. Disable sync in the account editor first.",
      rateSyncConflict:
        "Cannot change account rates: {count} target account(s) have upstream rate sync enabled.",
    ...
    createSparkShadowConfirm:
      'Create a spark shadow account linked to "{name}"? It shares the parent\'s credentials and serves only spark models.',
    ...
    duplicateSuccess:
      'Account duplicated as "{name}" and paused. Review its credentials before enabling it.',
    ...
    openai: {
      ...
      codexFingerprintModeDesc:
        "When multiple users share one OAuth account, converge device and session identifiers to stable account-level values to reduce the device and session count visible upstream. Off passes client identifiers through unchanged.",
      codexFingerprintOff: "Off (passthrough)",
      codexFingerprintDevice: "Device only",
      codexFingerprintSession: "Device and session (recommended)",
      ...
      codexImageToolInheritDesc:
        "No account override; hosted injection for non-Lite requests follows the channel or global policy, while client-provided hosted tools and local image_gen declarations pass through.",
    ...
    grok: {
      baseUrlHint:
        "Grok OAuth accounts forward to the official xAI API base URL.",
      apiKeyHint:
        "Grok subscription support uses OAuth refresh tokens; API keys are out of scope for this account type.",
      testMode: "Test mode",
      testModeHint:
        "Text, image, and video use the selected model. Web search, TTS, STT, and Realtime use standalone endpoint probes.",
      ...
      searchTestHint: "Standalone web search probe, not a free-form chat with tools.",
      ...
      ttsTestHint: "Calls standalone /v1/tts and displays the audio result.",
      sttTestHint: "Calls standalone /v1/stt; success means the endpoint is reachable.",
      realtimeTestHint: "Connects to /v1/realtime WebSocket; a successful handshake means it is reachable.",
      ...
      sendingRealtimeRequest: "Connecting to standalone /realtime WebSocket...",
      selectedTestMode: "Test mode: {mode}",
      imageUploadLabel: "Source image (optional for edits)",
      videoFirstFrameLabel: "First-frame or reference image (optional)",
      imageUploadHint:
        "PNG or JPEG is recommended. Upload a source image for image-to-image or editing; leave it empty for text-to-image.",
      videoFirstFrameHint: "Optional first-frame or reference image for image-to-video.",
      audioUploadLabel: "Audio file (optional for STT)",
      audioUploadHint: "Upload real audio to transcribe; leave it empty to test connectivity only.",
      mediaTooLarge: "File is too large; admin test uploads are limited to about 6 MB.",
    ...
    syncUpstreamModelsSuccess:
      "Synced {count} new model(s) from upstream ({total} upstream total)",
    syncUpstreamModelsNoChanges:
      "All {count} upstream model(s) are already in the whitelist",
    ...
    autoPauseThresholdHint:
      "Leave empty or set 0 to use the global default threshold (configured in Ops settings); set a value to override the global default. Reaching the threshold only skips the account during scheduling and does not modify schedulable.",
    autoPause5hDisabled: "Disable 5h auto-pause",
    autoPause7dDisabled: "Disable 7d auto-pause",
    autoPauseDisabledHint:
      "When enabled, this account is never auto-paused (even if a global default threshold is configured).",
    ...
    priority: "Call Priority",
    priorityHint: "Higher values are used first; equal values continue through load and platform scheduling rules, and 0 is the lowest priority",
    priorityInvalid: "Call priority must be a non-negative decimal integer within the database integer range",
    priorityColumnHint:
      "Higher values enter scheduling first. Equal values still use load, queue, recency, quota, and platform rules. This is not a traffic percentage and does not guarantee the same account is selected every time",
    priorityDecrease: "Decrease call priority",
    priorityIncrease: "Increase call priority",
    prioritySaving: "Saving call priority",
    priorityUpdateFailed: "Failed to save call priority",
    accountSchedulingThresholdOverride: "Account auto-pause threshold override",
    accountSchedulingThresholdOverrideHint:
      "Override the platform auto-pause threshold for this account only. Disable it to use the platform setting.",
    accountSchedulingThresholdOverrideValue: "Account threshold percent",
    accountSchedulingThresholdOverrideDisabledHint:
      "Enter 1 to 100. The account becomes temporarily unschedulable at this usage percentage; 100 disables auto-pause for this account.",
    billingRateMultiplier: "Billing Rate Multiplier",
    billingRateMultiplierHint: "0 = free, affects account billing only",
    expiresAt: "Expires At",
    expiresAtHint: "Leave empty for no expiration",
    higherPriorityFirst: "Higher value means higher call priority",
    ...
    oauth: {
      ...
      openai: {
        ...
        codexSessionDesc:
          "Paste a Codex OAuth auth.json or an accessToken. Accounts use the step 1 settings.",
        codexSessionInputLabel: "Codex OAuth auth.json or accessToken",
        codexSessionPlaceholder:
          "Multiple lines supported, one token or auth.json object per line",
        codexSessionHint:
          "OAuth session/access-token imports retain their existing expiration behavior.",
      ...
      grok: {
        ...
        step2OpenUrl: "Open the URL in your browser and complete authorization",
        openUrlDesc:
          "Open the authorization URL in a new tab, sign in to xAI, and authorize API access.",
        importantNotice:
          "When the browser reaches the local callback URL, copy the full URL or the code query parameter back here.",
        step3EnterCode: "Enter Authorization URL or Code",
        authCodeDesc:
          "After authorization, paste the callback URL, query string, or authorization code:",
        authCode: "Authorization URL or Code",
        authCodePlaceholder:
          "Paste the full callback URL, ?code=... query string, or code value",
        authCodeHint:
          "Full callback URLs, query strings, and bare codes are accepted.",
        refreshTokenAuth: "Manual RT Input",
        refreshTokenDesc:
          "Enter existing xAI refresh token(s). Supports batch input, one per line.",
        refreshTokenPlaceholder:
          "Paste your xAI refresh token...\nSupports multiple, one per line",
        ...
        emailPasswordDesc:
          "Sign in with a Grok web email and password. The server uses the password only to obtain temporary SSO and convert it to OAuth credentials; it stores neither the password nor raw SSO.",
        emailPasswordInputLabel: "email----password",
        emailPasswordPlaceholder:
          "email----password\nMultiple lines supported",
        emailPasswordHint:
          "Format: email----password; passwords may contain hyphens. YesCaptcha keys and a matching-region proxy are recommended.",
        pleaseEnterPassword: "Enter email----password, one account per line",
        pleaseEnterSSOToken: "Enter an SSO token",
        ...
        missingExchangeParams:
          "Missing authorization code, state, or OAuth session",
    ...
    gemini: {
      ...
      oauthType: {
        ...
        codeAssistRequirement:
          "Requires an active GCP project with billing enabled",
    ...
    videoPromptPlaceholder: "Example: A red ball bouncing once on a white floor.",
    videoPromptDefault:
      "A red ball bouncing once on a white floor, short simple motion.",
    videoTestHint: "Calls video generation and previews the completed result on the page.",
    ...
    usageWindow: {
      ...
      grokFreeQuota24hHint:
        "Estimated from local token usage over the rolling 24-hour window ({limit} limit)",
      grokWeeklyUsage: "Weekly {percent}%",
      grokUnknown:
        "Grok quota is unknown until the first upstream response includes xAI rate-limit headers.",
      grokRetryAfter: "Retry after {time}",
      grokProbe: "Probe",
      grokProbeTooltip:
        "Send a minimal xAI Responses probe and read quota headers",
      grokResetUnsupported: "Reset unsupported",
      grokResetUnsupportedTooltip:
        "xAI does not expose reset credits for Grok OAuth accounts",
    ...
    openaiQuotaReset: {
      ...
      resetTooltipReady:
        "Consume 1 reset credit to immediately restore the window",
      resetTooltipNeedQuery: "Click Credits first to load the available count",
      resetTooltipNoCredits: "No reset credits available",
      resetTooltipShadow:
        "Spark shadow accounts cannot reset credits; reset on the parent account",
      ...
      resetSuccess:
        "Reset {windows} window(s); credits and account state updated",
      resetCacheRefreshFailed:
        "The window was reset and account state recovered, but the reset-credit count could not be read back. Query it again.",
      resetAccountRecoveryFailed:
        "The window was reset, but account state recovery failed. Recover the account state manually.",
      resetAccountRefreshFailed:
        "The window, account state, and reset-credit cache were updated, but the latest account display could not be loaded.",
      refreshCachePersistFailed:
        "Showing the live count, but its expiration details were unavailable, so the cached details were kept.",
