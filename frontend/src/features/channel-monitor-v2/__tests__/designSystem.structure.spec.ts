...
describe('channel-monitor-v2 design system structure', () => {
  ...
  it('user ChannelStatus V2 shell uses the shared page, filter, metric, and table contracts', () => {
    ...
    expect(src).toContain(':aria-controls="expandedErrors.has(row.category) ? `monitor-error-details-${encodeURIComponent(row.category)}` : undefined"')
    expect(src).toContain(':id="`monitor-error-details-${encodeURIComponent(row.category)}`"')
    expect(src).toContain("candidate?.name === 'AbortError'")
    expect(src).toContain("candidate?.code === 'ERR_CANCELED'")
    expect(src).toContain('sequence += 1')
    expect(src).toContain('tabSequence += 1')
