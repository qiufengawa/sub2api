...
.ops-health__score{display:grid;width:100%;min-width:0;gap:8px}.ops-health__score-row{display:flex;align-items:baseline;gap:8px}.ops-health__score-row strong{font-size:28px;line-height:32px;font-variant-numeric:tabular-nums}.ops-health__score-row span{color:var(--ui-text-soft);font-size:11px}.ops-health__bar{width:100%;height:10px;overflow:hidden;border-radius:999px;background:var(--ui-surface-strong)}.ops-health__bar span{display:block;height:100%;border-radius:inherit;background:var(--ui-success);transition:background-color .25s ease}.ops-health__bar span.is-warning{background:var(--ui-warning)}.ops-health__bar span.is-danger{background:var(--ui-danger)}.ops-health__bar span.is-neutral{background:var(--ui-text-muted)}
...
.ops-traffic__grid{display:grid;min-width:0;grid-template-columns:repeat(2,minmax(0,1fr));gap:8px}.ops-traffic>.ui-button{justify-self:end}
