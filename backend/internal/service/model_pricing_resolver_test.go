...
func TestCalculateCostUnified_ChannelIntervalsRespectAccountLongContextGate(t *testing.T) {
	inputPrice := 1e-6
	longInputPrice := 2e-6
	outputPrice := 3e-6
	resolver := newResolverWithChannel(t, []ChannelModelPricing{{
		...
		BillingMode: BillingModeToken,
		Intervals: []PricingInterval{
			{MinTokens: 0, MaxTokens: testPtrInt(200000), InputPrice: &inputPrice, OutputPrice: &outputPrice},
			{MinTokens: 200000, InputPrice: &longInputPrice, OutputPrice: &outputPrice},
		},
	}})
	bs := resolver.billingService
	...
	disabled, err := bs.CalculateCostUnified(CostInput{
		Ctx: context.Background(), Model: "gpt-5.4", GroupID: groupIDPtr(), Group: group,
		Tokens: tokens, RateMultiplier: 1, Resolver: resolver, LongContextBillingEnabled: &off,
	})
	require.NoError(t, err)
	...
	enabled, err := bs.CalculateCostUnified(CostInput{
		Ctx: context.Background(), Model: "gpt-5.4", GroupID: groupIDPtr(), Group: group,
		Tokens: tokens, RateMultiplier: 1, Resolver: resolver, LongContextBillingEnabled: &on,
	})
	require.NoError(t, err)
	require.InDelta(t, float64(tokens.InputTokens)*inputPrice, disabled.InputCost, 1e-12)
	require.InDelta(t, float64(tokens.InputTokens)*longInputPrice, enabled.InputCost, 1e-12)
}
