...
func ProvideUsageBillingReservationCleanupService(repo UsageBillingRepository, cfg *config.Config) *UsageBillingReservationCleanupService {
	reservationRepo, _ := repo.(UsageBillingReservationRepository)
	svc := NewUsageBillingReservationCleanupService(reservationRepo, cfg)
	svc.Start()
	return svc
}
...
// ProvideSettingService wires SettingService with plan validation and proxy lookup.
func ProvideSettingService(settingRepo SettingRepository, entClient *dbent.Client, proxyRepo ProxyRepository, cfg *config.Config) *SettingService {
...
var ProviderSet = wire.NewSet(
	...
	ProvideUsageBillingReservationCleanupService,
...
func ProvidePaymentConfigService(entClient *dbent.Client, settingRepo SettingRepository, key payment.EncryptionKey, auth APIKeyAuthCacheInvalidator, billing *BillingCacheService) *PaymentConfigService {
	svc := NewPaymentConfigService(entClient, settingRepo, []byte(key))
	svc.SetCatalogImportCacheServices(auth, billing)
	return svc
}
