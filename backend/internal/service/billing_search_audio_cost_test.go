...
func TestCalculateSearchCost(t *testing.T) {
	t.Parallel()
	s := &BillingService{}
	require.Equal(t, 0.0, s.CalculateSearchCost(0, searchAudioFloatPtr(10), 1).ActualCost)
	// nil price → default $10/1k: 5 calls = 0.05
	require.InDelta(t, 0.05, s.CalculateSearchCost(5, nil, 1).ActualCost, 1e-9)
	// explicit 0 → free
	require.Equal(t, 0.0, s.CalculateSearchCost(5, searchAudioFloatPtr(0), 1).ActualCost)
	price := 10.0
...
func searchAudioFloatPtr(v float64) *float64 { return &v }
