...
func (s *RedeemService) invalidateRedeemCaches(ctx context.Context, userID int64, redeemCode *RedeemCode) {
	...
	case RedeemTypeSubscription:
		...
		if redeemCode.PlanID != nil && s.subscriptionService != nil {
			if coverageRepo, ok := s.subscriptionService.userSubRepo.(SubscriptionCoverageRepository); ok {
				if sub, err := coverageRepo.GetByUserIDAndPlanID(ctx, userID, *redeemCode.PlanID); err == nil {
					s.subscriptionService.maybeInvalidateAssignmentCaches(userID, subscriptionCoveredGroupIDs(sub), false)
				}
			}
		}
	}
...
func (s *RedeemService) reduceOrCancelSubscription(ctx context.Context, userID, planID int64, reduceDays int, code string) error {
	coverageRepo, ok := s.subscriptionService.userSubRepo.(SubscriptionCoverageRepository)
	if !ok {
		return ErrSubscriptionNotFound
	}
	sub, err := coverageRepo.GetByUserIDAndPlanID(ctx, userID, planID)
	if err != nil {
		return ErrSubscriptionNotFound
	}
