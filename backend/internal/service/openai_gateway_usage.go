...
func (s *OpenAIGatewayService) RecordUsage(ctx context.Context, input *OpenAIRecordUsageInput) error {
	...
	// Plan coverage determines the billing source; the API key group retains its own rate.
	...
	billingErr := func() error {
		_, err := applyUsageBilling(ctx, billingRequestID, usageLog, &postUsageBillingParams{
			Cost:                  cost,
			User:                  user,
			APIKey:                apiKey,
			Account:               account,
...
func (s *OpenAIGatewayService) calculateOpenAIRecordUsageCost(
	...
	tokens UsageTokens,
	serviceTier string,
	longContextBillingGate any,
) (*CostBreakdown, error) {
	gate := normalizeLongContextBillingGate(longContextBillingGate)
...
// normalizeLongContextBillingGate keeps older internal callers that passed a
// plain bool source-compatible while allowing nil for non-OpenAI platforms.
func normalizeLongContextBillingGate(value any) *bool {
	switch v := value.(type) {
	case nil:
		return nil
	case *bool:
