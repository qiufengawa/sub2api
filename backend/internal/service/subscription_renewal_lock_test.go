...
func (r *lockingRenewalRepo) GetActiveCoveringGroup(context.Context, int64, int64) (*UserSubscription, error) {
	panic("unexpected GetActiveCoveringGroup call")
}

func (r *lockingRenewalRepo) ListActiveCoveringGroup(context.Context, int64, int64) ([]UserSubscription, error) {
	panic("unexpected ListActiveCoveringGroup call")
}

func (r *lockingRenewalRepo) ExistsActiveCoveringGroup(context.Context, int64, int64) (bool, error) {
	panic("unexpected ExistsActiveCoveringGroup call")
}

func (r *lockingRenewalRepo) UpdateBillingSnapshot(_ context.Context, _ int64, snapshot SubscriptionBillingSnapshot, resetCycle bool) error {
	r.mu.Lock()
	defer r.mu.Unlock()
	r.billingSnapshotUpdates++
	r.current.PlanID = snapshot.PlanID
	if resetCycle {
		r.current.FiveHourStartedAt = snapshot.FiveHourStartedAt
		r.current.FiveHourUsageUSD = 0
		r.current.CycleStartedAt = snapshot.CycleStartedAt
		r.current.CycleUsageUSD = 0
		r.current.TotalUsageUSD = 0
	}
	return nil
}
...
func TestAssignSubscriptionDoesNotReactivateRowSuspendedAfterStaleRead(t *testing.T) {
	...
	require.Zero(t, repo.billingSnapshotUpdates)
}
