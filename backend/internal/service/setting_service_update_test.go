...
func TestSettingService_UpdateSettings_DefaultSubscriptions_ValidPlan(t *testing.T) {
	...
	planReader := &defaultSubPlanReaderStub{
		byID: map[int64]*dbent.SubscriptionPlan{
			11: {ID: 11, Edges: dbent.SubscriptionPlanEdges{Groups: []*dbent.Group{{ID: 101}}}},
		},
	}
	svc := NewSettingService(repo, &config.Config{})
...
func TestSettingService_UpdateSettings_DefaultSubscriptions_RejectsPlanWithoutGroups(t *testing.T) {
	repo := &settingUpdateRepoStub{}
	planReader := &defaultSubPlanReaderStub{
		byID: map[int64]*dbent.SubscriptionPlan{12: {ID: 12}},
	}
	svc := NewSettingService(repo, &config.Config{})
	svc.SetDefaultSubscriptionPlanReader(planReader)
	...
	require.Equal(t, "12", infraerrors.FromError(err).Metadata["plan_id"])
...
func TestSettingService_UpdateSettings_DefaultSubscriptions_RejectsMissingPlan(t *testing.T) {
	repo := &settingUpdateRepoStub{}
	planReader := &defaultSubPlanReaderStub{
		errBy: map[int64]error{13: ErrDefaultSubscriptionPlanInvalid},
	}
	svc := NewSettingService(repo, &config.Config{})
	svc.SetDefaultSubscriptionPlanReader(planReader)
...
func TestSettingService_UpdateSettings_DefaultSubscriptions_RequiresPlanReader(t *testing.T) {
	repo := &settingUpdateRepoStub{}
	svc := NewSettingService(repo, &config.Config{})
	...
	require.Error(t, err)
	require.Equal(t, "DEFAULT_SUBSCRIPTION_PLAN_READER_UNAVAILABLE", infraerrors.Reason(err))
	require.Nil(t, repo.updates)
}
