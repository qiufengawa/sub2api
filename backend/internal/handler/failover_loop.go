...
type FailoverState struct {
	*service.AccountFailoverState
	SwitchCount int
	MaxSwitches int
	// FailedAccountIDs is retained as a compatibility union for diagnostics and
	// older tests. Production scheduling uses AccountFailoverState so failure
	// reasons remain distinguishable.
...
func NewFailoverState(maxSwitches int, hasBoundSession bool) *FailoverState {
	return &FailoverState{
		AccountFailoverState:  service.NewAccountFailoverState(),
...
func (s *FailoverState) MarkSelectionRejected(accountID int64) {
	s.AccountFailoverState.MarkSelectionRejected(accountID)
	if accountID > 0 {
		s.FailedAccountIDs[accountID] = struct{}{}
	}
}

func (s *FailoverState) MarkRetryableRuntimeFailure(accountID int64) {
	s.AccountFailoverState.MarkRetryableRuntimeFailure(accountID)
	if accountID > 0 {
		s.FailedAccountIDs[accountID] = struct{}{}
	}
}

// RecordCapacityFailure records a request-time account capacity failure. A
// full wait queue or an account-slot wait timeout is retryable on another
// account, but must still respect the request's account-switch budget.
func (s *FailoverState) RecordCapacityFailure(accountID int64) FailoverAction {
	s.MarkRetryableRuntimeFailure(accountID)
...
func (s *FailoverState) RecordProfitVeto(accountID int64) FailoverAction {
	s.MarkSelectionRejected(accountID)
	s.profitVetoCount++
...
func (s *FailoverState) HandleSelectionExhausted(ctx context.Context) FailoverAction {
	...
	if s.LastFailoverErr != nil &&
		...
		if len(s.RetryableRuntimeFailedIDs) == 0 && s.SelectionRejectedCount() > 0 {
			logger.FromContext(ctx).Warn("gateway.failover_selection_exhausted_by_profit_veto",
				...
				zap.Int("excluded_accounts", s.ExcludedCount()),
			)
			return FailoverExhausted
		}
		...
		s.ClearRetryableRuntimeFailures()
		s.FailedAccountIDs = s.ExcludedIDs()
