...
RUN --mount=type=cache,id=sub2api-gomod,target=/go/pkg/mod \
    ...
    CGO_ENABLED=0 GOOS=${TARGETOS:-linux} GOARCH=${TARGETARCH} go build \
    -trimpath \
    -o /app/account-priority-publisher \
    ./cmd/account-priority-publisher
...
LABEL org.opencontainers.image.source="https://github.com/qiufengawa/sub2api"
...
COPY --from=backend-builder --chown=sub2api:sub2api /app/account-priority-publisher /app/account-priority-publisher
