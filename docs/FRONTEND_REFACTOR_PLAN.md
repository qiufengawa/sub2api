...
> 权威执行文档：根目录 [`plan.md`](../plan.md)。本文只补充页面信息架构、参考样式和用户端页面映射，不覆盖根计划的全站重构边界、批次顺序、存档规则或验收门禁。

> 全量重构决定：旧前端的样式、组件外观、页面 DOM、排版、布局和响应式补丁全部丢弃并重写；旧页面只用于提取业务契约。Git 存档分支 `archive/frontend-before-full-rebuild-20260814` 仅用于回退，不作为新页面视觉来源。

> 状态：R0-R9 全量长任务补充文档
>
> 执行口径：本文件只补充信息架构与视觉参考，不再定义“用户端阶段”或“管理员后续阶段”。管理员、用户、公共页面和支付流程均属于根目录 `plan.md` 的同一个 R0-R9 目标；任何阶段完成都不得停止，必须持续到发布与线上更新验证。
>
...
> 执行总纲：全站前端重构以仓库根目录 `plan.md` 为唯一执行计划。本文用于补充页面信息架构与视觉参考，不得被解释为保留旧页面样式、旧 DOM 或旧布局的理由。旧前端视觉实现全部删除重写；可保留的只有业务功能契约、API/store/composable、权限、路由、数据语义和状态机。

> 最新确认（2026-08-14）：这是一次全量前端重构。旧样式、旧组件外观、旧页面 DOM、旧排版、旧布局和旧响应式补丁全部淘汰；先使用 `archive/frontend-before-full-rebuild-20260814` 作为唯一 Git 回退点，再在 `ui/main` 逐页重写。除非 `plan.md` 的例外清单明确记录，否则不得把旧视觉实现带入新页面。

## 0. 与根计划的单一目标约束

根目录 [`plan.md`](../plan.md) 是唯一执行总纲；本补充文档不能被单独执行或单独验收。R0 基线、R1 公共组件、R2 应用壳、R3 用户核心、R4 支付、R5 开发工作区、R6 标准管理员、R7 复杂管理员、R8 公共页、R9 清理审计必须连续完成。只有最终自动化门禁、浏览器/无障碍门禁、Code Review、Release 产物与 checksum、GHCR/latest、旧版本更新发现和线上更新验证全部记录后，目标才算完成。
