...
历史版本 [`v1.0.0`](https://github.com/qiufengawa/sub2api/releases/tag/v1.0.0) 完成了独立发行源的建立。当前源码版本为 `v0.1.170-qiu.4`，版本号会同时记录上游基线与本发行分支的整数迭代序号。
...
```bash
docker pull ghcr.io/qiufengawa/sub2api:0.1.170-qiu.4
```
...
```bash
curl -sSL https://raw.githubusercontent.com/qiufengawa/sub2api/ui/main/deploy/install.sh | sudo bash -s -- --version v0.1.170-qiu.4
```
...
```yaml
image: ghcr.io/qiufengawa/sub2api:0.1.170-qiu.4
```
...
例如 `v0.1.170-qiu.1` 表示：

- 上游基线为 `Wei-Shaw/sub2api v0.1.170`；
- 这是该上游基线上的第 1 个整数迭代版本。

版本递增规则：

- 每次本分支改动递增整数序号，例如 `v0.1.169-qiu.3` → `v0.1.169-qiu.4`；
- 官方版本号更新时同步前半段，并将本分支版本重置为 `qiu.1`；
- 每个 Release 的说明必须分别列出“同步的上游内容”和“本分支改动”。

历史标签 `v1.0.0`、曾使用的 `v<上游版本>-v<发行版本>` 格式，以及已经发布的 `v0.1.169-qiu.3.1` 继续由更新与回滚逻辑识别；新的正式版本统一使用整数 `qiu.N`，以兼容旧版更新器。

完整的发布约束、兼容性检查和发布前后验收清单见 [发布版本规范](docs/RELEASE_VERSION_POLICY.md)。
