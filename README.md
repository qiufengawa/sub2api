...
历史版本 [`v1.0.0`](https://github.com/qiufengawa/sub2api/releases/tag/v1.0.0) 完成了独立发行源的建立。当前源码版本为 `v0.1.176-qiu.2`，版本号会同时记录上游基线与本发行分支的整数迭代序号。
...
```bash
docker pull ghcr.io/qiufengawa/sub2api:0.1.176-qiu.2
```
...
```bash
curl -sSL https://raw.githubusercontent.com/qiufengawa/sub2api/ui/main/deploy/install.sh | sudo bash -s -- --version v0.1.176-qiu.2
```
...
```yaml
image: ghcr.io/qiufengawa/sub2api:0.1.176-qiu.2
```
...
例如 `v0.1.176-qiu.2` 表示：

- 上游基线为 `Wei-Shaw/sub2api v0.1.176`；
- 这是该上游基线上的第 2 个整数迭代版本。
