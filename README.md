...
历史版本 [`v1.0.0`](https://github.com/qiufengawa/sub2api/releases/tag/v1.0.0) 完成了独立发行源的建立。当前源码版本为 `v0.1.172-qiu.1`，版本号会同时记录上游基线与本发行分支的整数迭代序号。
...
```bash
docker pull ghcr.io/qiufengawa/sub2api:0.1.172-qiu.1
```
...
```bash
curl -sSL https://raw.githubusercontent.com/qiufengawa/sub2api/ui/main/deploy/install.sh | sudo bash -s -- --version v0.1.172-qiu.1
```
...
```yaml
image: ghcr.io/qiufengawa/sub2api:0.1.172-qiu.1
```
...
例如 `v0.1.172-qiu.1` 表示：

- 上游基线为 `Wei-Shaw/sub2api v0.1.172`；
